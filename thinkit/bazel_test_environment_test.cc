...
TEST(BazelTestEnvironmentTest, StoreTestArtifact) {
  std::unique_ptr<TestEnvironment> environment =
      std::make_unique<BazelTestEnvironment>(/*mask_known_failures=*/true);
...
TEST(BazelTestEnvironmentTest, SetTestCaseIdCallsMemberFunction) {
  ...
  std::unique_ptr<TestEnvironment> environment =
      std::make_unique<BazelTestEnvironment>(
...
TEST(BazelTestEnvironmentTest, SaveSwitchLogsCallsMemberFunction) {
  MockFunction<void(absl::string_view)> save_switch_logs;
  EXPECT_CALL(save_switch_logs, Call("pretest_logs"));
  std::unique_ptr<TestEnvironment> environment =
      std::make_unique<BazelTestEnvironment>(
          /*mask_known_failures=*/true,
          /*set_test_case_ids=*/[](auto) {},
          /*save_switch_logs=*/save_switch_logs.AsStdFunction());
  environment->SaveSwitchLogs("pretest_logs");
}
...
// -- Benchmarks ---------------------------------------------------------------
//
// Best run with 'bazel test --test_arg=--benchmark_filter=all <target>'.
